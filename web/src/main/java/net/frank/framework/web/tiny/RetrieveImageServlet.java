...
public class RetrieveImageServlet extends HttpServlet {
	private static final long serialVersionUID = 703845944849872099L;
	private static final String RESPONSE_HEADER_DEFALUT_CONTENT_TYPE = "text/javascript";
	private static final String PROCESS_IMAGE_URL = "/attachment?resourceId=";
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Resource> imageList = queryService
				.retrieveResourceByType(CommonConstants.TYPE.IMAGE);
		List<Resource> filterList = permissionService.filterList(imageList,
		...
		for (Iterator<Resource> it = filterList.iterator(); it.hasNext();) {
			...
			if (permissionService.checkReadPermission(currResource,
				...
				item.setMapping("http://" + request.getServerName() + ":"
						...
						+ PROCESS_IMAGE_URL + currResource.getId());
				itemList.add(item);
			}
		}
		...
		pw.write("var tinyMCEImageList = new Array(");

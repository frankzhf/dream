 ...
 *	@since 0.8
 */
package net.frank.framework.web.taglib.tabs.listener;
...
public class TabServletContextListener implements ServletContextListener {
	...
	private static final String DEST_FOLDER_NAME = "/global/";
	public  static final String SCRIPT_URI = DEST_FOLDER_NAME + "script/tabs.js";
	public  static final String STYLE_URI  = DEST_FOLDER_NAME + "tabstyle.jsp";
	
	private static final List<String> DEST_PATHS = Arrays.asList(
			new String [] { SCRIPT_URI, STYLE_URI });
	
	private static final List<String> RESOURCE_PATHS = Arrays.asList(
		new String [] {"/lib/js/tabs.js", "/lib/style/tabs.css" });

	private static final String IMAGE_RESOURCE_FOLDER = "/lib/images/";
	
	private static final List<String> IMAGE_RESOURCE_NAMES = Arrays.asList(
	...
	public void contextInitialized(final ServletContextEvent evt) {
		...
		URL sourceURL;
		int i = 0;
		for (Iterator<String> iter = RESOURCE_PATHS.iterator(); iter.hasNext();) {
			sourcePath = iter.next();
			sourceURL  = getClass().getResource(sourcePath);
			destPath   = DEST_PATHS.get(i);
			destPath   = servletContext.getRealPath(destPath);
			writeFile(sourceURL,destPath,servletContext);
			i++;
		}
		for (Iterator<String> iter = IMAGE_RESOURCE_NAMES.iterator(); iter.hasNext();) {

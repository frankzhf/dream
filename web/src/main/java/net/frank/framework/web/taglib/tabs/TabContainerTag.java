...
public final class TabContainerTag extends SimpleTagSupport {
	...
	private String id;
	private String skin;
	private List<TabPaneTag> children;
	...
	private boolean loadOnSwitch = false;
	
	
	
	public boolean isLoadOnSwitch() {
		return loadOnSwitch;
	}

	public void setLoadOnSwitch(boolean loadOnSwitch) {
		this.loadOnSwitch = loadOnSwitch;
	}
	...
	List<TabPaneTag> getChildren() {
		if (null == children) {
			children = new ArrayList<TabPaneTag>();
		}
		return children;
	}
	...
	public void doTag() throws JspException, IOException {
		...
		int i = 0;
		for (Iterator<TabPaneTag> iter = children.iterator(); iter.hasNext();) {
			TabPaneTag tabPane = iter.next();
			out.startElement(Xhtml.Tag.DIV);
			...
			if (!isLoadOnSwitch()){
				...
				out.attribute(Xhtml.Attr.HREF,tabPane.getHref());
			}

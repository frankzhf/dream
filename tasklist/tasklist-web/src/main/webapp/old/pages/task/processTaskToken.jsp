...
$(function() {
	$("#content_12__clobString_text_").validatebox({   
	    required:true  
	});
...
function submitComment(){
	if($("#content_12__clobString_text_").validatebox('isValid')){
		...
		document.forms[0].action = '${pageContext.request.contextPath}/task/submitTaskTokenComment.do';
		$.submitFormWoMsg();
	}
}
function submitScreenShot(){
	if($("#updateFile_content_11").validatebox('isValid')){
		...
		document.forms[0].action = '${pageContext.request.contextPath}/task/submitTaskTokenScreenShot.do';
		$.submitFormWoMsg();
	}
}
function submitAttach(){
	if($("#updateFile_content_10").validatebox('isValid')){
		...
		document.forms[0].action = '${pageContext.request.contextPath}/task/submitTaskTokenAttach.do';
		$.submitFormWoMsg();
	}
}
</script>

<fk:content2 templateName="net.frank.framework.bo.viewTask.html" var="${task}"  mode="view"
...
<html:form action="/submitTaskTokenComment" enctype="multipart/form-data">
	...
	<html:hidden property="resource.longString(typeId)" styleId="resource_longString_typeId_" value="12"/>
	...
	<table width="100%">
		<tr>
			<td width="70%">
				<textarea id="content_12__clobString_text_" style="width:100%;" name="content(12).clobString(text)"></textarea>
			</td>
			<td valign="bottom">

...
public class BpmnParse extends Parse {
	...
	protected void parseSignals() {
		for (Element signalElement : rootElement.elements("signal")) {
			...
			String signalName = this.resolveName(signalElement
					.attribute("name"));
	...
	protected void parseAssociations(Element scopeElement, ScopeImpl parentScope) {
		for (Element associationElement : scopeElement.elements("association")) {
			...
			if (sourceActivity == null && !elementIds.contains(sourceRef)) {
				...
				if (sourceActivity.getProperty("type").equals(
	...
	public ActivityImpl parseBusinessRuleTask(Element businessRuleTaskElement,
			ScopeImpl scope) {
		ActivityImpl activity = createActivityOnScope(businessRuleTaskElement,
				scope);

		BusinessRuleTaskActivityBehavior ruleActivity = new BusinessRuleTaskActivityBehavior();

		String ruleVariableInputString = businessRuleTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "ruleVariablesInput");
		...
		String resultVariableNameString = businessRuleTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "resultVariable");
		...
		if (ruleVariableInputString != null) {
			...
			for (String ruleVariableInputObject : ruleVariableInputObjects) {
				ruleActivity.addRuleVariableInputIdExpression(expressionManager
						.createExpression(ruleVariableInputObject.trim()));
			}
		}
		...
		for (BpmnParseListener parseListener : parseListeners) {
			parseListener.parseBusinessRuleTask(businessRuleTaskElement, scope,
					activity);
		}
		return activity;
	}
			...
			for (Element taskListenerElement : taskListenerElements) {
				...
				if (eventName != null) {
					if (TaskListener.EVENTNAME_CREATE.equals(eventName)
						...
						addError(
								"Invalid eventName for taskListener: choose 'create' |'assignment'",
								userTaskElement);
					}
				} else {
	...
	protected TaskListener parseTaskListener(Element taskListenerElement) {
		...
		if (className != null) {
			...
			addError(
					"Element 'class' or 'expression' is mandatory on taskListener",
					taskListenerElement);
		}
		return taskListener;
	}
	...
	public void parseBPMNPlane(Element bpmnPlaneElement) {
		...
		if (processId != null && !"".equals(processId)) {
			...
			if (processDefinition != null) {
				...
				addError(
						"Invalid reference in 'bpmnElement' attribute, process "
								+ processId + " not found", bpmnPlaneElement);
			}
		} else {
	...
	public void parseBPMNShape(Element bpmnShapeElement,
		...
		if (activityId != null && !"".equals(activityId)) {
			...
			// bounds
			if (activity != null) {
				Element bounds = bpmnShapeElement.elementNS(
						BpmnParser.BPMN_DC_NS, "Bounds");
				if (bounds != null) {
					activity.setX(parseDoubleAttribute(bpmnShapeElement, "x",
							bounds.attribute("x"), true).intValue());
					activity.setY(parseDoubleAttribute(bpmnShapeElement, "y",
							bounds.attribute("y"), true).intValue());
					activity.setWidth(parseDoubleAttribute(bpmnShapeElement,
							"width", bounds.attribute("width"), true)
							.intValue());
					activity.setHeight(parseDoubleAttribute(bpmnShapeElement,
							"height", bounds.attribute("height"), true)
							.intValue());
				} else {
			...
			} else if (!elementIds.contains(activityId)) { // it might not be an
															// activity but it
															// might still
															// reference
															// 'something'
				...
				addError(
						"Invalid reference in 'bpmnElement' attribute, activity "
								+ activityId + "not found", bpmnShapeElement);
			}else{
				
				String poolName = bpmnShapeElement.attribute("bpmnElement");
				Element element = findElementByIdWithRecurse(getRootElement(),poolName);
				if(element.getTagName().equals("participant")){
					Element bounds = bpmnShapeElement.elementNS(
							BpmnParser.BPMN_DC_NS, "Bounds");
					if (bounds != null) {
						processDefinition.setX(parseDoubleAttribute(bpmnShapeElement, "x",
								bounds.attribute("x"), true).intValue());
						processDefinition.setY(parseDoubleAttribute(bpmnShapeElement, "y",
								bounds.attribute("y"), true).intValue());
						processDefinition.setWidth(parseDoubleAttribute(bpmnShapeElement,
								"width", bounds.attribute("width"), true)
								.intValue());
						processDefinition.setHeight(parseDoubleAttribute(bpmnShapeElement,
								"height", bounds.attribute("height"), true)
								.intValue());
					} else {
						addError("'Bounds' element is required", bpmnShapeElement);
					}
				}else if(element.getTagName().equals("lane")){
					Lane lane = new Lane();
					lane.setId(element.attribute("id"));
					lane.setName(element.attribute("name"));
					Element bounds = bpmnShapeElement.elementNS(
							BpmnParser.BPMN_DC_NS, "Bounds");
					if (bounds != null) {
						lane.setX(parseDoubleAttribute(bpmnShapeElement, "x",
								bounds.attribute("x"), true).intValue());
						lane.setY(parseDoubleAttribute(bpmnShapeElement, "y",
								bounds.attribute("y"), true).intValue());
						lane.setWidth(parseDoubleAttribute(bpmnShapeElement,
								"width", bounds.attribute("width"), true)
								.intValue());
						lane.setHeight(parseDoubleAttribute(bpmnShapeElement,
								"height", bounds.attribute("height"), true)
								.intValue());
					} else {
						addError("'Bounds' element is required", bpmnShapeElement);
					}
					processDefinition.getLaneSet().add(lane);
				}

...
public class ProcessDiagramCanvas {
	...
	protected static Color CONDITIONAL_INDICATOR_COLOR = new Color(255, 255,
			255);
	protected static Color LANE_COLOR = Color.BLUE;
	protected static Color HIGHLIGHT_COLOR = Color.RED;

	// Strokes
	protected static Stroke LANE_BORDER_STROKE = new BasicStroke(3.0f);
	...
	protected static Image START_IMAGE;
	protected static Image END_IMAGE;
	...
	static {
		try {
			START_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/start.png"));
			END_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/end.png"));
...
//		
	...
	public void drawNoneStartEvent(int x, int y, int width, int height) {
		drawStartEvent(x, y, width, height, START_IMAGE);
	}
	...
	public void drawNoneEndEvent(int x, int y, int width, int height) {
		...
		g.draw(new Ellipse2D.Double(x, y, width, height));
		g.setStroke(originalStroke);
		g.drawImage(END_IMAGE, x, y, width, height, null);
	}
	...
	protected String fitTextToHeight(String original, int height) {
		String text = original;
		int maxHeight = height - 10;
		while (fontMetrics.stringWidth(text + "...") > maxHeight
				&& text.length() > 0) {
			text = text.substring(0, text.length() - 1);
		}
		if (!text.equals(original)) {

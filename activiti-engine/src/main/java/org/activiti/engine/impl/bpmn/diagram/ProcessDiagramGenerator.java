...
public class ProcessDiagramGenerator {
	...
	protected static ProcessDiagramCanvas initProcessDiagramCanvas(
		...
		if (!processDefinition.getLaneSet().isEmpty()) {
			if (!processDefinition.getLaneSet().isEmpty()) {
				for (Iterator<Lane> it = processDefinition.getLaneSet()
						.iterator(); it.hasNext();) {
					Lane current = it.next();
					if (current.getX() + current.getWidth() > maxX) {
						maxX = current.getX() + current.getWidth();
					}
					if (current.getX() < minX) {
						minX = current.getX();
					}
					// height
					if (current.getY() + current.getHeight() > maxY) {
						maxY = current.getY() + current.getHeight();
					}
		...
		ProcessDiagramCanvas processDiagramCanvas = new ProcessDiagramCanvas(
				maxX + 10, maxY + 10, minX, minY);

		if (!processDefinition.getLaneSet().isEmpty()) {
			for (Iterator<Lane> it = processDefinition.getLaneSet().iterator(); it
					.hasNext();) {
				Lane current = it.next();
				processDiagramCanvas.drawLane(current.getName(),
						current.getX(), current.getY(), current.getWidth(),
						current.getHeight());
			}
		}
		return processDiagramCanvas;

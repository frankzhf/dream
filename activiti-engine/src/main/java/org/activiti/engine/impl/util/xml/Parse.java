...
public class Parse extends DefaultHandler {
	...
	protected void setStreamSource(StreamSource streamSource) {
		if (this.streamSource != null) {
			throw new ActivitiException("invalid: multiple sources "
					+ this.streamSource + " and " + streamSource);
		}
	...
	public Parse execute() {
		try {
			...
			if (schemaResource == null) { // must be done before parser is
											// created
			...
			throw new ActivitiException("couldn't parse '" + name + "': "
					+ e.getMessage(), e);
		}
	...
	public Element findElementById(Element element, String id) {
		if(element.attribute("id") != null && element.attribute("id").equals(id)){
			return element;
		}else{
			return null;
		}
	...
	public Element findElementByIdWithRecurse(Element parent, String id){
		Element returnValue = null;
		if(parent.attribute("id") != null && parent.attribute("id").equals(id)){
			return parent;
		}
		if(!parent.elements.isEmpty()){
			for(Iterator<Element> it = parent.elements.iterator();it.hasNext();){
				Element current = it.next();
				returnValue = findElementByIdWithRecurse(current, id);
				if(returnValue != null){
					return returnValue;
				}
			}

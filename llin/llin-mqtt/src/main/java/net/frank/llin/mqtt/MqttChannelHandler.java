...
final class MqttChannelHandler extends SimpleChannelInboundHandler<MqttMessage> {
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		MqttConnectPayload payload = new MqttConnectPayload(this.client.getClientConfig().getClientId(),
				...
				this.client.getClientConfig().getLastWill() != null
						...
						: null,
				this.client.getClientConfig().getUsername(), this.client.getClientConfig().getPassword());
	...
	private void handleConack(Channel channel, MqttConnAckMessage message) {
		...
		case CONNECTION_ACCEPTED:
			...
			this.client.getPendingSubscribtions().entrySet().stream().filter((e) -> !e.getValue().isSent())
					.forEach((e) -> {
	...
	private void handleSubAck(MqttSubAckMessage message) {
		MqttPendingSubscribtion pendingSubscribtion = this.client.getPendingSubscribtions()
		...
		for (MqttPendingSubscribtion.MqttPendingHandler handler : pendingSubscribtion.getHandlers()) {
			MqttSubscribtion subscribtion = new MqttSubscribtion(pendingSubscribtion.getTopic(), handler.getHandler(),
					handler.isOnce());
	...
	private void handlePublish(Channel channel, MqttPublishMessage message) {
		...
		case EXACTLY_ONCE:
			if (message.variableHeader().packetId() != -1) {
				...
				this.client.getQos2PendingIncomingPublishes().put(message.variableHeader().packetId(),
						incomingQos2Publish);
	...
	private void handleUnsuback(MqttUnsubAckMessage message) {
		MqttPendingUnsubscribtion unsubscribtion = this.client.getPendingServerUnsubscribes()
				.get(message.variableHeader().messageId());

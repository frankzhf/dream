...
<script
	src='${pageContext.request.contextPath}/dwr/interface/aclAjax.js'></script>

<script type="text/javascript">
	$(function() {
		$('#tabInfo').tabs({
			onSelect:function(title){
				if(title == "<bean:message key='core.details.tabs.history'/>"){
					var _tab = $(this).tabs('getSelected');
					$(this).tabs('update',{
						tab:_tab,
						options:{
							href:"${pageContext.request.contextPath}/core/listHistory.do?systemHistory.resourceId=${resource.id}"
						}
					});
				}else if(title == "<bean:message key='core.details.tabs.acl'/>"){
					var _tab = $(this).tabs('getSelected');
					$(this).tabs('update',{
						tab:_tab,
						options:{
							href:"${pageContext.request.contextPath}/core/listAcl.do?acl.resourceId=${resource.id}"
						}
					});
				}
			}
	...
	function showHistoryInfo(historyId){
		window.showModalDialog('${pageContext.request.contextPath}/core/viewHistoryInfo.do?systemHistory.id=' + historyId,
				'History Info', 'dialogWidth:800px;dialogHeight:600px');
				...
				<c:if test="${!empty resource}">
					<table style="width: 100%">
						...
						<tr>
							<td colspan="4"><c:if test="${ mode eq 'view' }">
								<fk:hasPermission permission="write" resourceId="${resource.id}">
					...
					<div id="tabInfo" style="position:absolute; padding:0px 5px;width:960px;height:480px">
						<c:if test="${3 eq resource.typeId}">
							<div title="<bean:message key='core.details.tabs.resource.list'/>">
						...
						<c:if test="${3 ne resource.typeId}">
							<div title="<bean:message key='core.details.tabs.details'/>">
								<fk:content var="${content}" typeResourceId="${resource.typeId}"
									mode="${mode}" />
							</div>
						</c:if>
						<div title="<bean:message key='core.details.tabs.system'/>">
							<table width="100%" class="listTable">
								<tr>
									<th width="20%">&nbsp;</th>
						...
						<div title="<bean:message key='core.details.tabs.acl'/>">
							
						</div>
						
						<div id="historyTab" title="<bean:message key='core.details.tabs.history'/>">

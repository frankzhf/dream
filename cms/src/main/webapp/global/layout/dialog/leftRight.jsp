...
<body class="easyui-layout">
	<div id="west" region="west" border="false" title="<bean:message key='${leftHeader}' />" split="true" style="width: 220px; padding: 5px;">
<c:if test="${empty forwordParam}">
		<iframe id="navigation" frameborder="0"  src="${pageContext.request.contextPath}<tiles:getAsString name='leftLinker'/>" style="width:100%;height:100%;"></iframe>
</c:if>
<c:if test="${!empty forwordParam}">
		<iframe id="navigation" frameborder="0"  src="${pageContext.request.contextPath}<tiles:getAsString name='leftLinker'/>${forwordParam}" style="width:100%;height:100%;"></iframe>
</c:if>
	</div>

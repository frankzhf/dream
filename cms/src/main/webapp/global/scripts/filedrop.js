...
fd.RandomID = function() {
	return 'fd_' + (Math.random() * 10000).toFixed();
};
fd.ByID = function(id) {
	return fd.IsTag(id) ? id : document.getElementById(id);
};
...
fd.NewXHR = function() {
	try {
		...
		var activex = new Array("MSXML2.XMLHTTP.6.0", "MSXML2.XMLHTTP.5.0",
...
fd.SetClass = function(element, className, append) {
	...
	if (fd.IsTag(element)) {
		if (typeof append == 'undefined' || append) {
			if (!fd.HasClass(element, className)) {
				element.className += ' ' + className;
			}
		} else {
			element.className = element.className.replace(fd
...
fd.CallAll = function(list, args, obj) {
	...
	if (fd.IsArray(list)) {
		...
		console
				.error('FileDrop event list must be either an Array or undefined/null,'
...
fd.DropHandle = function(zone, opt) {
	...
	if (!zone) {
		throw 'Cannot locate DOM node given to new FileDrop().';
	}
	self.opt = {
		zoneClass : 'fd-zone',
		inputClass : 'fd-file',
	...
	if (fd.isChrome) {
		self.opt.fullDocDragDetect = true;
	}
	self.on = {
		event : [],
		dragEnter : [],
	...
	self.Hook = function(node) {
		if (self.opt.input != false) {
			...
			if (self.opt.input) {
				fd.CallOf(self, 'inputSetup', self.opt.input);
			}
		}
	...
	self.HookDragOn = function(node) {
		function DoHook(event, toBody) {
			fd.AddEvent(toBody ? document.body : node, event.toLowerCase(),
					function(e) {
						fd.StopEvent(e);
	...
	self.PrepareInput = function(node) {
		var res = {
			file : self.FindInputRec(node) || self.CreateInputAt(node)
		};
		if (res.file) {
			var form = res.file.parentNode;
			while (form && !fd.IsTag(form, 'form')) {
				form = res.form.parentNode;
			}
			var target = form ? form.getAttribute('target') : '';
			if (target && fd.IsTag(fd.ByID(target), 'iframe')) {
				res.form = form;
			}
		}
		return res;
	...
	self.CreateInputAt = function(parent) {
		...
		// iframe code and several other things around are courtesy of QQ File
		// Uploader
		// (https://github.com/valums/file-uploader).
		cont.innerHTML = '<iframe src="javascript:false;" name="' + id
				+ '"></iframe>'
				+ '<form method="post" enctype="multipart/form-data">'
				+ '<input type="hidden" name="fd-callback" />'
				+ '<input type="file" name="fd-file" />' + '</form>';
		...
		// Opera doesn't recognize legend and put it on top of the fieldset
		// unless
		// it's the first child.
		while (nextChild
				&& (!fd.IsTag(nextChild) || fd.IsTag(nextChild, 'legend'))) {
			nextChild = nextChild.nextSibling;
		}
		if (nextChild) {
			// Firefox 10 requires that immediate parent has position: relative
			// for
			// overflow: hidden to work on the input; this in turn requires that
			// that
			// parent is the first child, otherwise top: 0 of the file input
			// won't work.
	...
	self.SendViaIFrame = function(url) {
		...
		if (url && form) {
			// IE will trigger <input>'s onchange on drop even though it won't
			// populate it.
			if (!('__proto__' in {})
					&& (!self.opt.input.file.value || self.opt.input.file.value.length < 3)) {
				return;
			}
			do {
				var cb = fd.RandomID();
			...
			window[cb] = function(resp) {
				var obj = typeof resp == 'object' ? resp : {
					response : resp,
					responseText : resp,
					readyState : 4,
					status : 200
			...
			while (cbInput
					&& (!fd.IsTag(cbInput, 'input') || cbInput.name != 'fd-callback')) {
...
fd.FileDrop = function(zone, opt) {
	...
	self.OnUpload = function(e) {
		...
		if (files == false) {
			self.handle.SendViaIFrame(); // no FileAPI/Chrome file features - use
			// <iframe>.
	...
	self.GetFilesFrom = function(e) {
		var list = e.dataTransfer ? e.dataTransfer : (e.target ? e.target.files
				: false);
		if (list == false) {
			return false;
		}
	...
	self.on.dragEnter.push(function() {
		self.SetClassesTo(zone, true);
	});
	self.on.dragLeave.push(function() {
		self.SetClassesTo(zone, false);
	});
	self.on.upload.unshift(function() {
		self.SetClassesTo(zone, false);
	});
	self.on.uploadElsewhere.push(function() {
		self.SetClassesTo(zone, false);
	});
...
fd.File = function(file) {
	...
	self.DoSendTo = function(url, e) {
		...
		if (self.opt.extraHeaders) {
			self.xhr.setRequestHeader('X-File-Name',
					encodeURIComponent(self.name));
			self.xhr.setRequestHeader('X-File-Size', self.size);
			var api = window.FileReader ? 'FileAPI' : 'Webkit';
			self.xhr
					.setRequestHeader('X-Requested-With', 'FileDrop-XHR-' + api);
		}
		var data = (e && e.target && e.target.result) ? e.target.result
				: self.nativeFile;
...
fd.jQuery = function($) {
	$ = $ || jQuery || window.$;
	if (!$) {
		throw 'No jQuery/$ object to integrate FileDrop into.';
	}
...
// If removed and a parenthesis follows (e.g. after concatenation with other
// scripts)

(function ($) {
    ...
    $.fn.dropdown = function (o) {
            ...
            parse: {
                ...
                render: function (parent) {
                    ...
                    if($.browser.msie && $.browser.version > 8){
                    	ul.css({"list-style":"none",margin:0,padding:0,border:"none"});
                    }
                    if (this.items && this.items.length > 0) {
                        $.each(this.items, function () {
                            ...
                            var li = $("<li/>");
                            if($.browser.msie && $.browser.version > 8){
                            	li.css({padding:"2px 4px",margin:0,"list-style":"none","white-space":"nowrap"});
                            }
        ...
        if($.browser.msie && $.browser.version > 8){
        	d = $("<div/>")
        		.css({"cursor":"default",padding:0,"overflow-x":"hidden !important","font-size":"14px",border:"1px solid #555","background-color":"#fff"})
        		.css({position: "absolute", "z-index": "999", "overflow": "auto", width: options.dropwidth, display: "none" })
        		.click(function (event) { event.stopPropagation(); })
        		.appendTo($("body"));
        }else{
        	d = $("<div/>").addClass(options.containerCssClass)
            	.css({ position: "absolute", "z-index": "999", "overflow": "auto", width: options.dropwidth, display: "none" })
            	.click(function (event) { event.stopPropagation(); })
            	.appendTo($("body"));
        }

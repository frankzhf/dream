$(document).ready(function(){
	/*
	  $("#components .component").attr("data-content", "<form class='form'>" +
		  		"<div>" +
		  		"<label>Label Text</label> <input class='input-large' type='text' name='label' id='label'>" +
		  		"<label>Name</label> <select id='name' name='name'></select>" +
		  		"<label>Required</label><input type='checkbox' name='required' id='required'/>" +
		  		"<label>ReadOnly</label><input type='checkbox' name='readonly' id='readonly'/>" +
		  		"<label>On Blur</label><textarea id='onblur' name='onblur'></textarea>" +
		  		"<label>On Change</label><textarea id='onchange' name='onchange'></textarea>" +
		  		"<label>On Click</label><textarea id='onclick' name='onclick'></textarea>" +
		  		"<label>On Focus</label><textarea id='onfocus' name='onfocus'></textarea>" +
		  		"<hr/>" +
		  		"<button class='btn btn-info'>Save</button><button class='btn btn-danger'>Cancel</button>" +
		  		"</div>" +
		  		"</form>");*/
	
	$("#target").delegate(".tabbable .nav-tabs .active", "click", function(e){	
		$(".popover").hide();
		var obj = $(this).find("a");
    var $this = $(this);
    var html = $this.html();
              ...
              $($temp.html()).insertBefore(tops[0]);
            	 ...
            	 $.each($("#target .section"), function(e){
                 	if($(this).position().top<mu_mouseY && $(this).position().top+$(this).height() > mu_mouseY){
                 		if($(this).find(".section-content .tabbable").length==0){
                 			$(this).find(".section-content").append($temp.append("\n\n\ \ \ \ ").html());
                 		}else{
                 			$(this).find(".tab-content .active").append($temp.append("\n\n\ \ \ \ ").html());
                 		}
                 	}
                 });
...
//    	});
    ...
    for(var index=0;index<arr.length;index++){
      ...
      if(valID === "#column"){
    	  ...
    	  val = $(e).attr("required");
    	  if(val==="required"){
    		  $(".popover " + valID).attr("checked",true);
    	  }
      ...
      else if(valID ==="#onclick"){
          val = $(e).attr("onclick");
          $(".popover " + valID).val(val);
        }
      else if(valID ==="#onblur"){
          val = $(e).attr("onblur");
          $(".popover " + valID).val(val);
        }
      else if(valID ==="#onfocus"){
          val = $(e).attr("onfocus");
          $(".popover " + valID).val(val);
        }
    ...
    $(".popover").delegate(".btn-info", "click", function(e){
      ...
      $.each($(".popover textarea"), function(i, e){
    	 inputs.push($(e)); 
      });
      $.each($(".popover select"), function(i, e){
     	 inputs.push($(e)); 
       });
      ...
      }else if(vartype==="name" || vartype==="onclick" || vartype==="onchange" || vartype==="onfocus" || vartype==="onblur"){
        $(value).attr(vartype, $(e).val());
      }
      else if(vartype==="placeholder"){
        $(value).attr("placeholder", $(e).val());
      } else if (vartype==="checkbox" || vartype==="required" || vartype==="readonly"){
        if($(e).is(":checked")){
          $(value).attr(vartype, true);
        }
        else{
          $(value).attr(vartype, false);
        }

...
public class LiveRecord {
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				JFrame frame = new LiveRecordFrame();
...
class LiveRecordFrame extends JFrame {

	/**
	 * 
	 */
	private static final long serialVersionUID = -9094444552853646720L;
	AudioFormat af = null;
	...
	public LiveRecordFrame() {
		...
		// 对开始录音按钮进行鼠标监听
		captureBtn.addMouseListener(new MouseListener() {
		...
		// 设置窗口上的图标
		// Image img =
		// this.getToolkit().getImage(getClass().getResource("/image/Recorder.jpg"));
	...
	public void save() {
		...
		try {
			// 以当前的时间命名录音的名字
			// 将录音的文件存放到F盘下语音文件夹下
			File filePath = new File("/Users/zhaofeng/Desktop/projects/frank/run/pig/AudioFile");
			String tarPath = "/Users/zhaofeng/Desktop/projects/frank/run/pig";
			...
			try {
				...
				CommandReturn rt = OsUtil.execute(" lame -b 16 " + filePath + File.separator + file.getName() + " "
						+ tarPath + File.separator + tarFile.getName());
				
				if(rt.getType() == RETURN_TYPE.SUCCESS){
					System.out.println("lame operate success.");
					if(file.exists()){
						file.delete();
					}
				}else{
				...
				//Process p = run.exec(filePath + "/" + "lame -b 16 " + filePath
				//		+ "/" + file.getName() + " " + tarPath
				...
				// //定义最终要存放的文件路径
				// String destPath =
				// "D:/Program Files/apache-tomcat-6.0.35/webapps/XWZ/tempFile/";
		...
		public void run() {
			...
			try {
				...
				while (stopflag != true) {
					...
					ais = new AudioInputStream(bais, af, copyBts.length
							/ af.getFrameSize());
					try {
						...
						intBytes = ais.read(audioDataBuffer, 0,
								audioDataBuffer.length);

						// 不写到混频器中这样就不会播放
						// if (intBytes >= 0) {
						// outBytes = sd.write(audioDataBuffer,
						// 0,audioDataBuffer.length);
						// }
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	class RecordWave extends JPanel implements Runnable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7316673863344335704L;
		...
		public void paint(Graphics g) {
			...
			if (audioDataBuffer != null) {
				...
				for (int i = 0; i < audioDataBuffer.length - 1; ++i) {
					...
					g.drawLine(i * jp2.getWidth() / 256,
							(int) audioDataBuffer[i] + 200, (i + 1)

							* jp2.getWidth() / 256,
							(int) audioDataBuffer[i + 1] + 200);
				}
			}
		}

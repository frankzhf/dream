	...
	return {
		...
		onOk : function()
		{
			...
			for(var i=0; i<videos.length; i++)
			{
				...
				if ( !video || !video.src )
					continue;
				innerHtml += '<cke:source src="' + video.src + '" type="application/x-mpegURL" />';
			...
			else
			{
				...
				var elementId = videoNode.getAttribute('id');
				var _script = CKEDITOR.dom.element.createFromHtml( '<script>var '+elementId+'_options = {hls: {withCredentials: true}};var player = videojs('+elementId+',{flash: '+elementId+'_options, html5: '+elementId+'_options});player.play();</script>', editor.document );
				div.append(_script);
			}
		},
		onHide : function()

...
<%@ attribute name="title" type="java.lang.String" required="false" description="对话框标题"%>
...
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		top.$.jBox("iframe:"+url, {title: "${title}", width: 1000, height: 740, buttons:{'关闭': true,"确定":"ok"},
			submit:function(v, h, f){
				if(v=="ok"){
					var _ckfinderApi = h.find("iframe")[0].contentWindow.getCkfinderCallbackApi();
					...
					for(var i=0; i<files.length <c:if test="${! selectMultiple}"> && i<1 </c:if>; i++){
						var _url;
